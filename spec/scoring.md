...
- **value** = `0.7*(rating/5) + 0.3*affordability`, where affordability = `1 − price_norm`
...
- **gem_signal** = `rating_factor * scarcity * value`, with `rating_factor = clamp((rating−4)/1)`
...
`{"scores":[{id, value, location, character, gem_signal, rationale}]}` via the `openai` SDK
against `Settings.llm_base_url` (`response_format=json_object`, `temperature=0`); validates with
Pydantic; **one repair retry** on bad JSON. It **falls back to `HeuristicScorer`** if: no
`llm_api_key`, any exception, or the response omits any hotel id. Overall is computed locally.

Provider-agnostic by design: any OpenAI-compatible endpoint, selected via env. **This project's
LLM endpoint is Nebius Token Factory** (`https://api.tokenfactory.nebius.com/v1/`, Bearer key),
reached by the same `LLMScorer` with no code change; the code default still targets Groq's free
tier and switching is an env change, not code. Endpoint, key, model, and timeout are all in
`config.md`.
